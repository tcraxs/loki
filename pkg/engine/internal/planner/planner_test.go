...
func (t *TestMetastore) DataObjects(ctx context.Context, start time.Time, end time.Time, matchers ...*labels.Matcher) ([]string, error) {
	panic("unimplemented")
}

// Labels implements metastore.Metastore.
func (t *TestMetastore) Labels(ctx context.Context, start time.Time, end time.Time, matchers ...*labels.Matcher) ([]string, error) {
	panic("unimplemented")
}

// Sections implements metastore.Metastore.
func (t *TestMetastore) Sections(ctx context.Context, start time.Time, end time.Time, matchers []*labels.Matcher, predicates []*labels.Matcher) ([]*metastore.DataobjSectionDescriptor, error) {
	dur := end.Sub(start)
...
func (t *TestMetastore) StreamIDs(ctx context.Context, start time.Time, end time.Time, matchers ...*labels.Matcher) ([]string, [][]int64, []int, error) {
	panic("unimplemented")
}

// Streams implements metastore.Metastore.
func (t *TestMetastore) Streams(ctx context.Context, start time.Time, end time.Time, matchers ...*labels.Matcher) ([]*labels.Labels, error) {
	panic("unimplemented")
}

// Values implements metastore.Metastore.
func (t *TestMetastore) Values(ctx context.Context, start time.Time, end time.Time, matchers ...*labels.Matcher) ([]string, error) {
	panic("unimplemented")
}
...
└── Compat src=metadata dst=metadata collision=label
    └── SortMerge column=builtin.timestamp order=DESC
        ├── DataObjScan location=objects/00/0000000000.dataobj streams=5 section_id=1 projections=() direction=DESC limit=1000 predicate[0]=GTE(builtin.timestamp, 2025-01-01T00:00:00Z) predicate[1]=LT(builtin.timestamp, 2025-01-01T01:00:00Z)
        └── DataObjScan location=objects/00/0000000000.dataobj streams=5 section_id=0 projections=() direction=DESC limit=1000 predicate[0]=GTE(builtin.timestamp, 2025-01-01T00:00:00Z) predicate[1]=LT(builtin.timestamp, 2025-01-01T01:00:00Z)
				`,
		},
		{
...
└── Filter predicate[0]=EQ(ambiguous.label_foo, "bar")
    └── Compat src=metadata dst=metadata collision=label
        └── SortMerge column=builtin.timestamp order=DESC
            ├── DataObjScan location=objects/00/0000000000.dataobj streams=5 section_id=1 projections=() direction=DESC limit=0 predicate[0]=GTE(builtin.timestamp, 2025-01-01T00:00:00Z) predicate[1]=LT(builtin.timestamp, 2025-01-01T01:00:00Z) predicate[2]=MATCH_STR(builtin.message, "baz")
            └── DataObjScan location=objects/00/0000000000.dataobj streams=5 section_id=0 projections=() direction=DESC limit=0 predicate[0]=GTE(builtin.timestamp, 2025-01-01T00:00:00Z) predicate[1]=LT(builtin.timestamp, 2025-01-01T01:00:00Z) predicate[2]=MATCH_STR(builtin.message, "baz")
						`,
		},
		{
