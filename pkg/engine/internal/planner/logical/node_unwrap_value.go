...
type UnwrapValue struct {
	...
	UnwrapOperation UnwrapOp // Optional operation (e.g., "duration_seconds", "bytes")
}
...
func (t UnwrapOp) String() string {
	...
	case UnwrapDurationSeconds:
		return "unwrap duration_seconds"
	default:
		panic(fmt.Sprintf("unknown unwrap operation %d", t))
	}
}

func parseUnwrapOp(op string) UnwrapOp {
	switch op {
	case "unwrap":
		return Unwrap
	case "unwrap bytes":
	...
	case "unwrap duration_seconds":
		return UnwrapDurationSeconds
	default:
